...
class PlayerStats:
    ...
    def from_dict(cls, name: str, data: dict[str, Any]) -> PlayerStats:
        ...
        fpl = data.get("fpl_position") or _infer_fpl_position(pos)
        seasons = list(data.get("seasons_used", []))
        teams_by_season = dict(data.get("teams_by_season", {}))
        if not teams_by_season and data.get("team") and seasons:
            teams_by_season = {s: data["team"] for s in seasons}
        return cls(
            player=name,
            team=data.get("team", ""),
            primary_position=pos,
            fpl_position=fpl,
            positions=data.get("positions", [pos]),
            minutes=float(data.get("minutes", 0)),
            games=int(data.get("games", 0)),
            starts=int(data.get("starts", 0)),
            goals90=float(data.get("goals90", 0)),
            assists90=float(data.get("assists90", 0)),
            xg90=float(data.get("xg90", 0)),
            xa90=float(data.get("xa90", 0)),
            shots90=float(data.get("shots90", 0)),
            shots_on_target90=float(data.get("shots_on_target90", 0)),
            key_passes90=float(data.get("key_passes90", 0)),
            tackles90=float(data.get("tackles90", 0)),
            interceptions90=float(data.get("interceptions90", 0)),
            clearances90=float(data.get("clearances90", 0)),
            dribbles90=float(data.get("dribbles90", 0)),
            dribble_pct=float(data.get("dribble_pct", 0)),
            passes_completed90=float(data.get("passes_completed90", 0)),
            pass_pct=float(data.get("pass_pct", 0)),
            long_balls90=float(data.get("long_balls90", 0)),
            long_ball_pct=float(data.get("long_ball_pct", 0)),
            big_chances_created90=float(data.get("big_chances_created90", 0)),
            big_chances_missed90=float(data.get("big_chances_missed90", 0)),
            possession_lost90=float(data.get("possession_lost90", 0)),
            penalty_goals90=float(data.get("penalty_goals90", 0)),
            npxg90=float(data.get("npxg90", 0)),
            xg_chain90=float(data.get("xg_chain90", 0)),
            xg_buildup90=float(data.get("xg_buildup90", 0)),
            understat_key_passes90=float(data.get("understat_key_passes90", 0)),
            understat_matched=bool(data.get("understat_matched", False)),
            saves90=float(data.get("saves90", 0)),
            goals_prevented90=float(data.get("goals_prevented90", 0)),
            goals_conceded90=float(data.get("goals_conceded90", 0)),
            clean_sheet_pct=float(data.get("clean_sheet_pct", 0)),
            yellow_cards90=float(data.get("yellow_cards90", 0)),
            red_cards90=float(data.get("red_cards90", 0)),
