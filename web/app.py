...
class AdminRecordMatchResultRequest(BaseModel):
    """Admin endpoint to manually record a match result with scorers/assisters."""
    home_goals: int
    away_goals: int
    events: list[dict[str, Any]] = Field(
        default_factory=list,
        description="List of goal/assist events: {team, player, event_type (goal/assist), minute, assister?}"
    )
...
@app.post("/api/admin/tournament/{tournament_id}/matches/{match_id}/record-result")
def admin_record_match_result(
    tournament_id: str,
    match_id: str,
    body: AdminRecordMatchResultRequest,
    x_admin_token: str | None = Header(default=None, alias="X-Admin-Token"),
) -> dict:
    """Admin endpoint: manually record a match result with scorers/assisters.

    Stores the result in the database (if enabled) for retroactive recording.
    This is separate from the simulation override — it's for recording actual matches
    that already happened and need to be added to the stats board.
    """
    _check_admin(x_admin_token)
    try:
        import db

        # Load tournament to get team names
        t = tournament.load_tournament(tournament_id)
        if not t:
            raise HTTPException(status_code=404, detail="Tournament not found")

        match = tournament._find_fixture(t, match_id)
        if not match:
            raise HTTPException(status_code=404, detail="Match not found")

        fixture_info = match[1]
        home_team = fixture_info.get("home", "")
        away_team = fixture_info.get("away", "")
        stage = t.get("format")
        group_key = fixture_info.get("group")

        if not db.is_db_enabled():
            raise HTTPException(
                status_code=503,
                detail="Database not enabled. Configure DATABASE_URL on Render."
            )
        ...
        # Clear old events and record new ones
        db.clear_match_events(tournament_id, match_id)
        for event in body.events:
            if not isinstance(event, dict):
                continue
            team = event.get("team", "").strip()
            player = event.get("player", "").strip()
            event_type = event.get("event_type", "goal").lower()
            minute = event.get("minute")
            assister = event.get("assister", "").strip() if event_type == "goal" else None

            if team and player:
                db.record_match_event(
                    tournament_id,
                    match_id,
                    team,
                    player,
                    event_type,
                    minute=minute,
                    assister=assister if assister else None,
                )

        # Fetch and return the recorded data
        result = db.get_match_result(tournament_id, match_id)
        events = db.get_match_events(tournament_id, match_id)
        ...
        return {
            ...
            "result": result,
            "events": events,
            "message": f"Recorded {home_team} {body.home_goals}-{body.away_goals} {away_team} with {len(events)} event(s)",
        }
        ...
        raise HTTPException(status_code=500, detail=f"Failed to record match: {exc}") from exc


@app.get("/api/admin/tournament/{tournament_id}/matches/{match_id}/recorded-result")
def admin_get_recorded_match_result(
    tournament_id: str,
    match_id: str,
    x_admin_token: str | None = Header(default=None, alias="X-Admin-Token"),
) -> dict:
    """Admin endpoint: retrieve a manually recorded match result and its events."""
    _check_admin(x_admin_token)
    try:
        import db
        ...
        result = db.get_match_result(tournament_id, match_id)
        events = db.get_match_events(tournament_id, match_id) if result else []
...
@app.get("/api/admin/tournament/{tournament_id}/recorded-results")
def admin_list_tournament_recorded_results(
    tournament_id: str,
    x_admin_token: str | None = Header(default=None, alias="X-Admin-Token"),
) -> dict:
    """Admin endpoint: list all manually recorded match results for a tournament."""
    _check_admin(x_admin_token)
    try:
        import db

        if not db.is_db_enabled():
            return {"tournament_id": tournament_id, "results": [], "message": "Database not enabled"}

        results = db.get_tournament_match_results(tournament_id)
